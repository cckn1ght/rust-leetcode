...
pub fn get_problem_by_stat(stat: &StatWithStatus) -> Option<Problem> {
    if stat.paid_only {
        None
    } else {
        ...
        let resp: RawProblem = client
            ...
            .json(&Query::question_query(
                stat.stat.question_title_slug.as_ref().unwrap(),
            ))
            .send()
            .unwrap()
            .json()
            .unwrap();
        Some(Problem {
            title: stat.stat.question_title.clone().unwrap(),
            title_slug: stat.stat.question_title_slug.clone().unwrap(),
...
pub struct StatWithStatus {
    pub stat: Stat,
    pub status: Option<String>, // can be None、"ac" 、"notac"
